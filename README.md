...
- IPv6 (optional): asked at install; the lxdbr0 bridge gets the global prefix (/64 or shorter, incl. provider /80 slices; no NAT). Each container derives a deterministic /112 block from its username (sha256 → 32-bit block index); LXD routes the whole block to the container and ndppd relays Neighbor Discovery on the upstream side, so any address the container binds is reachable from the internet. The block is computed on the fly, never stored. `vpsmgr show` / the panel display each container's IPv6 block.
